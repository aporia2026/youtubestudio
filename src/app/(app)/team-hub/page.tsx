...
/**
 * /team-hub — owner-side cockpit for managing every team member in one
 * place. Three-pane layout (left rail | middle command center | right
 * slide-in surface). URL-driven state so deep links work.
 *
 * URL contract:
 ...
 * "Add person" flow (commit 8) and on window focus so unread/last-access
 * counters stay reasonably fresh without a full poll.
 */
export default function TeamHubPage() {
  ...
  const updateUrl = useCallback(
    (next: { person?: string | null; tab?: TeamHubTab; surface?: SurfaceDescriptor | null }) => {
      ...
      router.replace(qs ? `/team-hub?${qs}` : '/team-hub', { scroll: false });
    },
    [router, searchParams],
  );
