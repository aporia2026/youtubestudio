...
export function buildPanelFillPrompt(args: BuildPanelFillPromptArgs): PanelFillPrompt {
  ...
  const system = [
    ...
    '',
    'MOTION DELTA — keep each panel-to-panel change TINY. This is the second-biggest failure mode after element-scale:',
    '  Each consecutive panel should advance the motion by a SMALL increment — one limb segment moving a short distance, a head turning a few degrees, a prop sliding only a fraction of the scene. The image model has to reproduce composition every panel; the smaller the delta you ask for, the more faithfully it preserves the background, the static characters, the props, the framing. Big deltas (panel 1 = at start, panel 4 = at end with no in-between) force the model to re-imagine the whole composition every step, which is exactly what makes motion look jumpy.',
    '',
    '  It is BETTER for the final panel to show the motion ALMOST complete than for each panel to leap 1/N of the way. A subtle near-completion reads as smoother motion than four big jumps.',
    '',
    '  ANTI-PATTERN (4 panels, "George runs across"): "George at start position" → "George 1/3 across" → "George 2/3 across" → "George at end position". Each step is a huge composition change. The image model treats every panel as an independent reroll.',
    '',
    '  GOOD PATTERN (4 panels, same motion): "George at start, right foot beginning to lift" → "right foot in mid-air just past the start mark" → "right foot landing one stride ahead, left foot beginning to lift" → "left foot in mid-air, George now two strides past start". Each step is one small body-mechanics increment. The final panel does not need to reach the truck — it needs to be smaller-than-you-expect.',
    '',
    '  Rule of thumb: if you can describe the per-panel delta in fewer than 8 words ("foot lifts", "head turns a few degrees", "label fades in a bit more"), the delta is the right size. If your delta needs a long sentence ("character now most of the way across the room"), the delta is too big.',
    '',
    ...
    '  - Spread the motion across the panels with TINY deltas (see MOTION DELTA above) — favor smooth, near-complete motion over evenly-divided jumps.',
