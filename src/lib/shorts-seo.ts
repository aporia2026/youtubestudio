...
const YT_MAX_TAGS = 30;
const YT_MAX_TAG_LENGTH = 100;
const YT_MAX_COMBINED_LENGTH = 500;
 ...
 *  per-tag), and caps the whole array at 30 tags. Finally drops from
 *  the END until combined-length fits the 500-char total budget — the
 *  LLM orders by relevance so trimming the tail preserves the highest-
 *  value tags. Allows internal whitespace (multi-word phrases are the
 *  whole point of tags vs hashtags). */
export function normaliseYoutubeTags(raw: unknown): string[] {
  ...
  for (const item of raw) {
    ...
    const cleaned = item.trim().replace(/^#+/, '').replace(/\s+/g, ' ').slice(0, YT_MAX_TAG_LENGTH);
    if (!cleaned) continue;
