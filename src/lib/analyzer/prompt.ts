...
export function buildAnalyzerPrompt(input: BuildAnalyzerPromptInput): BuiltAnalyzerPrompt {
  ...
  const user = [
    ...
    'GUIDANCE',
    '',
    `1. style_packs is the heart of the output. A typical video has multiple visual modes — for example a Veritasium video usually has at least three: "talking-head" (host on camera), "broll" (cinematic supporting footage), and "animated-explainer" (diagrams). Identify every distinct mode that occupies ≥10% of the runtime OR appears more than twice. Use kebab-case ids like "talking-head", "broll", "animated-explainer", "screen-recording", "cinematic-narrative". Emit ONE pack per mode. Short videos with a single consistent look emit ONE pack. NEVER emit zero packs.`,
    '',
