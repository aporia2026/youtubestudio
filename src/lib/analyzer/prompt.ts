...
export function buildAnalyzerPrompt(input: BuildAnalyzerPromptInput): BuiltAnalyzerPrompt {
  ...
  const user = [
    ...
    '',
    `12. Scene and chapter boundaries must be in real video seconds — measured against the video's actual runtime, not estimated narratively. This is the single most common mistake observed in prior outputs: time fields summing to far more seconds than the video actually contains.\n\nHARD CONSTRAINTS (these are arithmetic, not stylistic):\n- scenes[0].start MUST be 0.\n- scenes[last].end MUST equal meta.duration_seconds (the video's actual total runtime). Tolerance: ±2 seconds for rounding ONLY. A deviation of more than 2 seconds is a bug.\n- Each scene's start MUST equal the previous scene's end. No gaps, no overlaps.\n- The sum of (scene.end - scene.start) across all scenes MUST equal meta.duration_seconds.\n- The same constraints apply to transcript.chapters: chapters[0].start = 0, chapters[last].end = meta.duration_seconds within ±2s, chapter starts/ends are contiguous. Chapters often diverged from scenes in prior outputs; that is a bug — both must match the same runtime.\n\nWORKED EXAMPLE — a 60-second video correctly segmented into three scenes:\n  { duration_seconds: 60, scenes: [{start:0,end:18,...}, {start:18,end:42,...}, {start:42,end:60,...}] }\n  Last scene ends at exactly 60. Sum of durations is 18+24+18 = 60. This is the only correct shape.\n\nCOUNTER-EXAMPLE — what NOT to produce for a 60-second video:\n  { duration_seconds: 60, scenes: [{start:0,end:20,...}, {start:20,end:50,...}, {start:50,end:95,...}] }\n  Last scene ends at 95 when the video is 60 seconds long. This is the pattern this rule exists to prevent.\n\nSCENE GRANULARITY: emit FINE-GRAINED scenes, one per distinct shot or composition change. Aim roughly for one scene every 5-15 seconds for a typical mid-paced video; fast-cut content (handheld vlogs, action montages) gets more scenes; a static lecture gets fewer. For a 5-minute video that is roughly 12-30 scenes; for a 10-minute video roughly 20-50. Do NOT collapse a long montage into one giant scene just to be safe on arithmetic — if you can't bound an individual scene's exact end within ±2s, round to the nearest 5s and emit the scene anyway. Coarse-but-safe is worse than granular-and-approximately-right. The arithmetic constraint above is still hard; the granularity guidance applies within it.\n\nIf only one scene clearly exists (e.g. a 30-second static talking-head), emit one scene from 0 to meta.duration_seconds. NEVER let scenes overflow meta.duration_seconds — this is a hard arithmetic constraint, not a preference.\n\nstyle_packs[i].occupies_seconds will be recomputed server-side from the scenes that reference each pack, so prioritise accurate scene boundaries over manually computing pack runtimes.`,
    '',
