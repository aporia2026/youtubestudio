...
export function buildAnalyzerPrompt(input: BuildAnalyzerPromptInput): BuiltAnalyzerPrompt {
  ...
  const user = [
    ...
    '',
    `12. Scene boundaries must reflect real video seconds. scenes[0].start MUST be 0. scenes[last].end MUST equal meta.duration_seconds within ±2 seconds for rounding. Scenes are non-overlapping (each scene's start equals the previous scene's end). The sum of all scene durations MUST equal meta.duration_seconds. Do NOT stretch, compress, or interpolate scene durations to make the narrative feel more even. If you are uncertain where a scene ends, prefer cutting at chapter or section breaks rather than guessing a duration. style_packs[i].occupies_seconds will be recomputed server-side from the scenes that reference each pack, so prioritise accurate scene boundaries over manually computing pack runtimes.`,
    '',
