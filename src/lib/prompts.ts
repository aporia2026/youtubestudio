...
\`\`\`
  ...
  - \`variant_edit_prompt\` — REQUIRED on derived rows (variant_index > 0): the smallest possible delta from the base ("raise the right eyebrow", "open the mouth into an O shape"). LEAVE \`ai_image_prompt\` EMPTY on derived rows — the server composes the final prompt from the base's ai_image_prompt + this delta. The BASE row keeps the full ai_image_prompt and omits variant_edit_prompt. **When the variant changes the NUMBER of people / figures / characters in the scene, write the delta in PRESERVATION form (state the target count: "Show exactly 6 figures total"), NOT in SUBTRACTION form ("remove the extra two children"). Subtraction-style prompts produce unreliable counts. See the chosen style's COUNT-PRECISE VARIANTS section for the canonical wrong/right patterns.**
  - \`variant_derives_from_previous\` (OPTIONAL boolean) — set to \`true\` on derived rows when the variant should edit the PREVIOUS variant's image instead of the base. Use for groups whose narration describes a continuous motion arc (door opens → swings → slams; hand rises → halfway → fully up). When omitted (default), variants are siblings — each edits the base independently. Cap chained groups at 3 variants (chain depth 3). The base row (variant_index 0) does not get this field. See the style's chained-variants mixing_rules for when to use it.
