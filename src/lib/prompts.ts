...
/**
 * Resolved style payload accepted by `productionDocPrompt`.
 *
 * Mirrors `ResolvedStyle` from `@/lib/production-doc-styles` but is duplicated
 * here so prompts.ts stays free of any DB or registry import — keep the two
 * shapes in sync.
 */
...
export function productionDocPrompt({
  ...
  /** Resolved style (built-in entry). Pass null/undefined for "no style". */
