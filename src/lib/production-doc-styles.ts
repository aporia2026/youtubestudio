...
export const BUILT_IN_STYLES: readonly ResolvedStyle[] = Object.freeze([
   ...
   */
  {
    ...
    mixing_rules: [
      ...
      '',
      'TARGET RATIO — ~40% of rows MUST belong to variant groups, ~60% are fresh compositions. THIS IS A FLOOR YOU SHOULD AIM ABOVE, NOT A CEILING TO STAY UNDER. Undershoot is the common failure mode: LLMs default to "every beat gets its own fresh scene" because each beat reads as logically distinct. That habit produces a slideshow of unrelated pictures and breaks the evolving-picture feel that defines this style. The viewer should feel scenes BREATHE — a held composition unfolding across 2-4 narration beats — not snap to a new picture every line.',
      '',
      ...
      '',
      'UNDER-USE is the real failure mode. If you finish the doc with ZERO motion_collage rows on a script that contains 2+ trigger verbs, you missed them. Go back, find the triggers, convert those rows.',
      '',
      'CADENCE: aim for ~1 motion_collage row per every 2 hard-trigger verbs in the script. A typical 19-row mystery/disaster doc with 3-4 motion beats should produce 2-3 motion_collage rows. A static historical doc with no motion verbs produces zero — that\'s fine.',
      '',
      'WHEN TO USE:',
      ...
      '  • The composition, character, camera, background, and lighting are IDENTICAL across every panel — ONLY the moving element advances. Repeat the static parts in each panel prompt so the model has them reliably; the model will keep them stable across panels anyway, but redundancy helps consistency.',
      '  • Describe the moving element\'s STATE at THIS panel, not the action ITSELF.',
      '    GOOD: "the runner\'s right foot is mid-stride, left foot is planted on the brown ground"',
      ...
      '  "visual_description": "Four-panel motion collage of the Vasa heeling left in stages, from upright to dangerous angle.",',
      ...
      '    "Same wide harbor view, same ship and water and sky and crew; the ship now tilted 20 degrees to port, masts leaning left.",',
      '    "Same wide harbor view; the ship now tilted 50 degrees to port, deck almost vertical, crew sliding toward the rail.",',
      '    "Same wide harbor view; the ship now tilted 75 degrees to port, masts touching the water, crew clinging to ropes."',
      '  ]',
      '}',
      '```',
      '',
