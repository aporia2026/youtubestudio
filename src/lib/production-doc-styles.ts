...
export const BUILT_IN_STYLES: readonly ResolvedStyle[] = Object.freeze([
   ...
   */
  {
    ...
    mixing_rules: [
      ...
      '  • When in doubt, pick `stick`. The cheap fallback for a row that needs a recurring entity but does not fit a Mode B world is `stick` plus a `zenn_character_id`.',
      '',
      'MODE PICK REASON — every row that sets `zenn_mode` must also set `zenn_mode_reason` to a SHORT (≤ 20 words) one-sentence explanation of why this mode is right for this beat. The field is purely diagnostic — the renderer and pipeline both ignore it — but it lets the user grep the doc for misclassifications when a render looks off. Examples: "abstract feeling of being watched, no specific entity to draw" (stick), "named historical figure recurring across the next four beats" (scene), "chart callout, no character" (stick).',
      '',
      ...
      '',
      'CANVAS REVEAL — For Mode A rows over 5 seconds, evolve the canvas across the shot instead of holding a still. Emit a `zenn_canvas_reveal_layers` array on the row; each entry is one element that fades onto the existing image at the timecode you set. The pipeline generates the sibling-frame PNG for each entry via Kie i2i Edit from the row\'s base image and writes the URL back, so you LEAVE `image_url` empty and only set the four LLM fields. Shape per entry:',
      '  • `prompt_hint`  (string, required) — what to ADD to the canvas. One short sentence: "draw a second mouse on the right side, same style". Avoid restating elements already in the base image — the model preserves them automatically.',
      '  • `reveal_at_ms` (number, required) — milliseconds from row start when the layer begins appearing. Space layers ~600-1200ms apart for the genre\'s readable cadence.',
      '  • `duration_ms`  (number, optional) — how long the layer stays visible. Omit to mean "until end of shot" (the typical case for canvas_reveal, since layers accumulate).',
      '  • `fade_in_ms`   (number, optional) — cross-fade window length. Omit for the default 250ms canvas_reveal feel. Set to `0` for the snappy `canvas_layer_add` semantics — an instant pop-in for "thing appears" beats like an arrow, a checkmark, or a date stamp.',
      'Cap: server-side limit of 4 layers per row to bound Kie Edit cost (~$0.05/layer). The pipeline silently drops layers beyond the cap. Reserve canvas_reveal for the long Mode A shots that need the evolving-canvas device; cutting harder with `zenn_mode: "stick"` static rows is cheaper and often reads better.',
      '',
      'CONCRETE EXAMPLE — narrator-mascot says "Every five seconds, someone in the world is born with this":',
      '  {',
      '    "timecode": "0:24",',
      "    \"zenn_mode\": \"stick\",",
      "    \"zenn_mode_reason\": \"narrator-mascot delivering a statistic, no scene world\",",
      ...
      '    "timecode": "1:12",',
      "    \"zenn_mode\": \"scene\",",
      "    \"zenn_mode_reason\": \"named historical figure recurring across this whole segment in a Kalahari outdoor setting\",",
      ...
      '  }',
      '',
      'CONCRETE EXAMPLE — Mode A canvas_reveal that grows the cage population in the Calhoun-style mouse-utopia segment:',
      '  {',
      '    "timecode": "2:48",',
      "    \"zenn_mode\": \"stick\",",
      "    \"zenn_mode_reason\": \"long Mode A explainer beat; layers accumulate to dramatize population growth\",",
      "    \"ai_image_prompt\": \"[two cartoon mice sitting on a grey floor inside a square cage, neutral expressions]\",",
      "    \"zenn_canvas_reveal_layers\": [",
      "      { \"prompt_hint\": \"add a third mouse on the right, same drawing style as the existing two\", \"reveal_at_ms\": 600 },",
      "      { \"prompt_hint\": \"add two more mice in the upper-left, same style, slightly smaller to suggest depth\", \"reveal_at_ms\": 1800 },",
      "      { \"prompt_hint\": \"add a bold red 600+ label above the cage in hand-lettered style\", \"reveal_at_ms\": 3400, \"fade_in_ms\": 0 }",
      "    ]",
      '  }',
      '',
