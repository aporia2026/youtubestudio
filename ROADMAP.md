...
Promoted on 2026-05-03 after a multi-agent audit of Phases 1–7 (security, data-integrity, errors/observability, performance, frontend) surfaced 30 findings. Phase 8.1–8.4 closes the audit out across 11 commits already shipped on `phase-1-foundation`. 8.5 is the new-theme work that the audit cleared the runway for.

**Migration map (Phase 8):** 0037 (composite indexes), 0038 (competitor_channels workspace UNIQUE), 0039 (published_videos idempotency_key).
...
| 8.5 | **AI improvement loop — post-publish prediction outcomes** — close the loop on Phase 4.2's retention predictor by storing actual-vs-predicted deltas after each video's first 14 days live, feeding the worst predictions back as few-shot examples so the model gets sharper on this user's specific channel over time | ⏸ | Theme: every YouTube tool *predicts*; none *learn from outcome*. Migration TBD — `prediction_outcomes` table (workspace_id, retention_prediction_id, video_analytics row reference, predicted_curve, actual_curve, delta_metrics jsonb, captured_at). New cron `/api/cron/capture-prediction-outcomes` runs daily, finds video_analytics rows for videos that went live ≥14 days ago + had a retention_prediction at publish time, stores the delta. Updates `retention-predictor.findFewShotExamples` to prefer recent outcomes over raw analytics rows. Surfaces "your scripts are getting better at hooks but worse at midroll retention over the last 30 days" insight on `/dashboard`. The data is already there (video_analytics.retention_curve + retention_predictions + scripts) — just need the linkage table + cron + dashboard panel. |
