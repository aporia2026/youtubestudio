...
- **YouTube upload quota**: `videos.insert` costs 1,600 units; default daily quota is 10,000 units → **~6 uploads/day per Google Cloud project**, hard ceiling. `publishAt` scheduling does not change this — the quota is charged at upload time, not publish time. Strategies: spread uploads across days via scheduler, OR request quota increase from Google (form + 1–3 week review). MVP assumes default quota; UI surfaces remaining quota and warns when a batch would exceed it.
...
- **YouTube field hard limits**: title ≤ 100 chars, description ≤ 5000 chars, tags combined ≤ 500 chars (including comma separators per YouTube's accounting), category required, `madeForKids` (COPPA) required.
